...
const MAX_VISIBLE_NEWS = 240;

function newsIdentity(news: NewsItem) {
  return news.url || `${news.source}-${news.title}`;
}

function mergeLiveNews(previous: NewsItem[], incoming: NewsItem[]) {
  const previousByIdentity = new Map(previous.map((item) => [newsIdentity(item), item]));

  return incoming.slice(0, MAX_VISIBLE_NEWS).map((item) => {
    const previousItem = previousByIdentity.get(newsIdentity(item));
    if (!previousItem) return item;

    return {
      ...item,
      id: previousItem.id,
      image: previousItem.hasImageError ? previousItem.image : item.image,
      hasImageError: previousItem.hasImageError,
    };
  });
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchNews = async (background = false) => {
      ...
      try {
        const res = await fetch(`/api/news?ts=${Date.now()}`, {
          cache: "no-store",
        });
        ...
        if (!isMounted) return;
        setNewsList((prev) => mergeLiveNews(prev, incoming));
        setFetchError(null);
      } catch (err: any) {
        if (!background && isMounted) setFetchError(err.message);
      } finally {
        isFetching = false;
    ...
    const interval = window.setInterval(() => {
      if (document.visibilityState !== "hidden") fetchNews(true);
    }, NEWS_REFRESH_INTERVAL_MS);
    const refreshOnFocus = () => fetchNews(true);
            ...
            >
              ...
              >
                ...
                <g transform="translate(6 8)">
                  <circle cx="18" cy="58" r="6.5" fill={`url(#${uid}-copper)`} />
                  <path
                    d="M 18 38 C 32 42 42 52 45 67"
                    fill="none"
              ...
              <div
                className="font-display font-bold text-[#1a273b] dark:text-slate-100 uppercase tracking-tight relative top-[6px] -ml-1 flex items-center gap-2"
                style={{
                  fontSize: "2rem",
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex items-center h-[50px] origin-left">
                  ...
                  >
                    ...
                    <g transform="translate(6 8)">
                      <circle cx="18" cy="58" r="6.5" fill={`url(#${uid}-copper-foot)`} />
                      <path
                        d="M 18 38 C 32 42 42 52 45 67"
                        fill="none"
                  ...
                  <div
                    className="font-display font-bold text-[#1a273b] dark:text-slate-100 uppercase tracking-tight relative top-[5px] -ml-1"
                    style={{
                      fontSize: "1.6rem",
                      textShadow: isDarkMode
                        ? "none"
